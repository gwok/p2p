        ...
        var login = {
            ...
            strVerify: function(event) {
                ...
                if (strName == 'userName') {
                    ...
                    if ((strVal.trim()).length < 6 || (strVal.trim()).length > 24) {
                        $(ids).text("");
                        $(ids).append("<span style=color:#ff7800>用户名不得小于6位或者大于24位</span>");
                        return false;
                    } else if (!/^[a-zA-Z][a-zA-Z0-9_]{5,23}$/.test(strVal)) {
                        $(ids).text("");
                        ...
                        $.ajax({
                            type: "post",
                            dataType: "text",
                            url: "/ssoweb/user/ifexist.do", //发送请求地址
                            data: {
                                verifyStr: strName,
                                verifyVal: strVal
                            },
                ...
                if (strName == 'password') {
                    ...
                    if ((strVal.trim()).length < 6 || (strVal.trim()).length > 24) {
                        $(ids).text("");
                        $(ids).append("<span style=color:#ff7800>密码不得小于6位或者大于24位</span>");
                        return false;
                    }
                    if (!/^(?![0-9]+$)(?![a-zA-Z]+$)[0-9A-Za-z]{5,23}$/.test(strVal)) {
                        $(ids).text("");
    ...
    $.ajax({
        ...
        success:function (data) {
            ...
            if(data['errorCode']=="000"){
                loginError.text("用户名密码错误")
            }
            if(data['errorCode']=="001"){
                loginError.text("未知错误")
            }
            if(data['errorCode']=="002"){
                loginError.text("图片验证码错误")
            }
            if(data['errorCode']=="003"){//成功登录
                location.href="/displayweb/index.html";
            }
            alert(data);
        },
        error:function (data) {
...
function change() {
    ...
    img.src="http://localhost:8080/piccodeweb/verify/get.do?t="+(new Date()).getTime();
}